...
#define METALLIB_PATH "build/07_matmul_kloop/kernel.metallib"
#endif
...
static void setup() {
    ...
    id<MTLFunction> fn = [lib newFunctionWithName:@"matmul_kloop"];
...
static id<MTLCommandBuffer> dispatch(const Bufs& b, int M, int N) {
    // C must be zero before dispatch (multiply_accumulate starts from C).
    memset([b.C contents], 0, M*N*4);
...
int main() {
    ...
    }
    {
        ...
        r.print("07 matmul kloop (static tilek=32)", 2.0*M*N*K);
    }
    return 0;
}
