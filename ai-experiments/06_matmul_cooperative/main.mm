// 06_matmul_cooperative – host driver
// matmul + bias add fused via cooperative_tensor.
...
#define METALLIB_PATH "build/06_matmul_cooperative/kernel.metallib"
#endif
...
static void setup() {
    ...
    id<MTLFunction> fn = [lib newFunctionWithName:@"matmul_bias_cooperative"];
...
static id<MTLCommandBuffer> dispatch(const Bufs& b, int M, int N) {
    ...
    [enc setBuffer:b.bias offset:0                  atIndex:3];
    [enc setBuffer:b.p    offset:0*sizeof(uint32_t) atIndex:4];
    [enc setBuffer:b.p    offset:1*sizeof(uint32_t) atIndex:5];
    [enc setBuffer:b.p    offset:2*sizeof(uint32_t) atIndex:6];
...
int main() {
    ...
    printf("GPU: %s\n", [gDevice.name UTF8String]);
    {
        ...
        std::vector<float> hBias(N);
        for (int i=0;i<M*K;++i) hA[i]=f32_to_f16((i%7)*0.1f);
        for (int i=0;i<K*N;++i) hB[i]=f32_to_f16((i%5)*0.2f);
        for (int i=0;i<N;++i)   hBias[i]=(float)(i%13)*0.5f;
        std::vector<float> ref(M*N);
        cpu_ref(hA,hB,hBias,ref,M,N,K);
        Bufs b = makeBuffers(M,N,K,hA,hB,hBias);
    ...
    }
    {
        ...
        r.print("06 matmul+bias fused (cooperative_tensor)", 2.0*M*N*K);
    }
    return 0;
}
