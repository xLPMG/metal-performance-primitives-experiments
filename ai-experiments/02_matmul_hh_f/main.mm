// 02_matmul_hh_f – host driver
// half × half → float (NN). Identical dispatch to 01 but C is float32.
...
#define METALLIB_PATH "build/02_matmul_hh_f/kernel.metallib"
#endif
...
static void setup() {
    ...
    id<MTLFunction> fn = [lib newFunctionWithName:@"matmul_hh_f"];
...
int main() {
    ...
    }
    {
        ...
        r.print("02 matmul half×half→float", 2.0*M*N*K);
    }
    return 0;
}
